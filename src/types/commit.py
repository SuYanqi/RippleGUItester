...
from src.types.file import File, Files
from src.utils.datetime_util import DatetimeUtil
from src.utils.dict_util import DictUtil
from config import FIREFOX_COMMIT_MESSAGE_LINK, FIREFOX_COMMIT_MESSAGE_JSON_LINK, FILE_CONTENT_LINK, FILE_CONTENT_JSON_LINK, \
...
class FilePatch:
    def __init__(self, file_id=None, filepath=None,
                 ...
                 patch_contents=None, previous_filepath=None):
        # self.commit = commit  # object
        self.file_id = file_id
        self.filepath = filepath  # Current file path
        self.prev_filepath = previous_filepath  # Previous path if the file was renamed
        self.status = status  # added, removed, modified (rename and/or change content)
    ...
    @staticmethod
    def get_hg_prev_current_filepaths_status(file_patch_dict):
        """
        Extracts the previous and current file paths from a Mercurial (HG) file patch dictionary.
        Args:
            file_patch_dict (dict): Dictionary containing information about file changes in the patch.
        Returns:
            tuple: A tuple containing the previous and current file paths, and the file status.
        cannot process with png file
        """
        previous_filepath = ''
        current_filepath = ''
        status = None

        lines = file_patch_dict['lines']
        # Check for rename operation first
        if len(lines) > 1:
            # rename + optional (change content)
            rename_pair = ('rename from', 'rename to')
            if lines[0]['l'].startswith(rename_pair[0]) and lines[1]['l'].startswith(rename_pair[1]):
                previous_filepath = lines[0]['l'].removeprefix(rename_pair[0]).strip()
                current_filepath = lines[1]['l'].removeprefix(rename_pair[1]).strip()
                status = Placeholder.FILE_MODIFIED
            # Check for copy operation (which should not be treated as rename)
            # copy + optional (change content)
            elif lines[0]['l'].startswith('copy from') and lines[1]['l'].startswith('copy to'):
                previous_filepath = lines[0]['l'].removeprefix('copy from').strip()
                current_filepath = lines[1]['l'].removeprefix('copy to').strip()
                status = Placeholder.FILE_COPY

        # If no rename or copy, look for file path indicators in diff lines
        if not previous_filepath and not current_filepath:
            for line in lines[:6]:  # Adjust the range if needed
                if line['t'].startswith('-') and line['l'].startswith('--- '):
                    previous_filepath = line['l'].removeprefix('--- a/').strip() if line['l'].startswith(
                        '--- a/') else ''
                elif line['t'].startswith('+') and line['l'].startswith('+++ '):
                    current_filepath = line['l'].removeprefix('+++ b/').strip() if line['l'].startswith(
                        '+++ b/') else ''
        if not status:
            status = Placeholder.FILE_MODIFIED
        if not current_filepath and previous_filepath:
            status = Placeholder.FILE_REMOVED
        elif not previous_filepath and current_filepath:
            status = Placeholder.FILE_ADDED
        elif not previous_filepath and not current_filepath:
            status = None
        return previous_filepath, current_filepath, status
    ...
    def get_hg_filepatch_list(commit_dict):
        ...
        for index, file_patch_dict in enumerate(file_patch_dict_list):
            # print(file_patch_dict)
            previous_filepath, current_filepath, status = FilePatch.get_hg_prev_current_filepaths_status(file_patch_dict)
            file_patch = FilePatch.from_hg_dict(file_patch_dict)
            # @todo
            file_patch.filepath = current_filepath
            file_patch.prev_filepath = previous_filepath
            file_patch.status = status
    ...
    def get_file_patch_text(self):
        file_patches_text = ''
        for patch_content in self.patch_contents:
            file_patches_text += patch_content.text
        return file_patches_text
...
class Commit:
    ...
    def __init__(self, id=None, message=None, comment_count=None, date=None,
                 api_url=None, html_url=None, comments_url=None, commit_parents=None,
                 # commit_children=None,
                 files=None, file_patches=None, push_date=None):
        self.id = id
        self.message = message
        self.comment_count = comment_count
        self.date = date  # for hg is the creation time,
                          # for github is the commit time, there is also a verification time named verified_at, but we don't use this
        ...
        # self.commit_children = commit_children
        self.files = files  # object: use file_dicts and then convert into objects @todo modified is ok, but deleted and added not exacely due to rename
        self.file_patches = file_patches
        ...
        try:
            ...
            if with_file_patch:
                ...
                return cls(commit_dict['sha'], commit_dict['commit']['message'], commit_dict['commit']['comment_count'],
                           ...
                           commit_dict['parents'],
                           # commit_dict['children'],
                           commit_dict['files'], file_patches)
            return cls(commit_dict['sha'], commit_dict['commit']['message'], commit_dict['commit']['comment_count'],
                       commit_date,
                       commit_dict['url'], commit_dict['html_url'], commit_dict['comments_url'],
                       commit_dict['parents'],
    ...
    def from_hg_dict(cls, commit_dict, with_file_patch=False):
        ...
        try:
            ...
            file_patches = None
            if with_file_patch:
                # @todo check filepatch
                file_patches = FilePatch.get_hg_filepatch_list(commit_dict)
                # print("OK")
            return cls(commit_dict['node'], commit_dict['desc'], None,
                       commit_date,
                       f'{FIREFOX_COMMIT_MESSAGE_JSON_LINK}{commit_dict["node"]}',
                       f'{FIREFOX_COMMIT_MESSAGE_LINK}{commit_dict["node"]}', None,
                       commit_dict['parents'],
                       # commit_dict['children'],
                       commit_dict['files'], file_patches, push_date)
        except Exception as e:
            # print(f"{e}: {commit_dict}")
            print(f"{e}")
            return None
    ...
    def get_hg_commit_parent_file_content_urls(self):
        ...
        for commit_parent_id in self.commit_parents:
            for file_patch in self.file_patches:
                # NEW added file has no commit_parent_id version
                if file_patch.status is not Placeholder.FILE_ADDED:
    ...
    def get_hg_commit_parent_file_annotate_urls(self):
        ...
        for commit_parent_id in self.commit_parents:
            for file_patch in self.file_patches:
                # new added file doesn't have file annotate
                if file_patch.status is not Placeholder.FILE_ADDED:
...
class Commits:
    ...
    def get_commit_by_html_url(self, url):
        for commit in self.commits:
            if url in commit.html_url:
                return commit
        return None
    ...
    def get_files(self):
        files = Files()
        for commit in self.commits:
            files.from_commit_with_cochanges(commit)
        return files
