...
from src.types.cochange import CoChanges, CoChange
...
class Content:
    ...
    @classmethod
    def from_dict(cls, file, content, encoding='base64'):
...
class File:
    def __init__(self, id=None, filepath=None, last_commit=None, content=None,
        ...
        self.path = filepath  # WordPress/src/main/java/org/wordpress/android/ui/mysite/SiteNavigationAction.kt
    ...
    @classmethod
    def from_file_content_dict(cls, file_content_dict):
        file = cls()
        try:
            file.path = file_content_dict['path']
            # @todo
            match= re.search(r"ref=([a-f0-9]{40})", file_content_dict['url'])
            file.last_commit = match.group(1)
            file.api_url = file_content_dict['url']
            file.html_url = file_content_dict['html_url']
            file.git_url = file_content_dict['git_url']
            file.download_url = file_content_dict['download_url']
            file.type = file_content_dict['type']
            file.name = file_content_dict['name']
            file.size = file_content_dict['size']
            # try:
            file.content = Content.from_dict(file, file_content_dict['content'])
            return file
        except Exception as e:
            # print(file_content_dict)
            print(f"from_hg_file_content_dict is wrong: {file_content_dict}\nException: {e}")
    ...
    def add_cochange(self, file):
        # Check if the file is already in the cochanges list
        for cochange in self.cochanges:
            if cochange.file.path == file.path:
                # Add commit only if it's not already in the list
                if file.last_commit not in cochange.commits:
                    cochange.commits.append(file.last_commit)
                return

        # If not, create a new CoChange object and append it
        cochange = CoChange(file)
        cochange.commits.append(file.last_commit)
    ...
    def get_file_content(self):
        ...
        file_content = ""
        for line in self.content.lines:
            file_content = file_content + line.content + '\n'
        return file_content
    # @classmethod
...
class Files:
    ...
    @classmethod
    def from_file_content_dicts(cls, file_content_dicts):
        files = []
        try:
            # @todo
            match = re.search(r"ref=([a-f0-9]{40})", file_content_dicts[0]['url'])
            ...
            pass
        for file_content_dict in file_content_dicts:
            file = File.from_file_content_dict(file_content_dict)
            if file:
                files.append(file)
        return cls(files, last_commit)
    ...
    def remove_file_by_filepath(self, filepath):
        target_file = self.get_file_by_filepath(filepath)
        if not target_file:
            return

        # Remove reverse co-changes from other files
        if target_file.cochanges:
            for cochange in target_file.cochanges:
                cochange_file = cochange.file
                cochange_file.cochanges = [
                    rc for rc in (cochange_file.cochanges or [])
                    if rc.file.path != target_file.filepath
                ]

        # Finally, remove the target file from the collection
    ...
    def get_modified_deleted_lines_by_test_commit(self, test_commit):
        ...
        for file_patch in test_commit.file_patches:
            ...
            file = self.get_file_by_filepath(file_patch.path)
            # print(file)
            if file:
    ...
    def add_cochanges_among_files_in_commit(self):
        for file_1 in self.files:
            for file_2 in self.files:
                if file_1.path != file_2.path:
                    file_1.add_cochange(file_2)

    def from_commit_with_cochanges(self, commit):
        files_in_commit = []
        for file_patch in commit.file_patches:
            # print(file_patch)
            if file_patch.status == Placeholder.FILE_ADDED or file_patch.status == Placeholder.FILE_COPY:
                file = File(filepath=file_patch.filepath, last_commit=commit)
                self.append(file)
                files_in_commit.append(file)
                # print("added or copy")
            elif file_patch.status == Placeholder.FILE_REMOVED:
                self.remove_file_by_filepath(file_patch.filepath)
            elif file_patch.status == Placeholder.FILE_MODIFIED:
                if file_patch.filepath != file_patch.prev_filepath:
                    file = self.get_file_by_filepath(file_patch.prev_filepath)
                    if file:
                        file.rename(file_patch.filepath)
                    else:
                        file = File(filepath=file_patch.filepath, last_commit=commit)
                        self.append(file)
                        files_in_commit.append(file)
                ...
                if file is None:
                    ...
                    file.last_commit = commit
                files_in_commit.append(file)
            # print(len(files_in_commit))
            ...
            #     print(file.path)
            # print("***********************************************************")
        Files(files_in_commit).add_cochanges_among_files_in_commit()
