...
class NLPUtil:
    ...
    @staticmethod
    def convert_str_into_bool(true_or_false_str):
        if type(true_or_false_str) is not bool:
            if true_or_false_str.lower() == 'true':
                true_or_false_str = True
            elif true_or_false_str.lower() == 'false':
                true_or_false_str = False
            else:
                raise ValueError(f"Invalid literal for boolean: '{true_or_false_str}'")
    ...
    def is_url(string):
        # print(string)
        parsed_url = urlparse(str(string))
        # print(parsed_url)
