...
if __name__ == "__main__":
    ...
    pulls = FileUtil.load_json(Path(filepath, f"test_pull_dicts.json"))
    print(f"all pulls: {len(pulls)}")

    merged_pulls = []
    for one in pulls:
        if one["pull_request"]["merged_at"]:
    ...
    FileUtil.dump_json(Path(filepath, f"test_merged_pull_dicts.json"), merged_pulls)
